// SERVER-ONLY. 0G Compute — TEE-verified image generation. Ported from demo/run-aura.ts.
...
import { createZGComputeNetworkBroker } from "./zg-compute";
...
/** Construct the compute broker for a signer (the demo wallet pays for generation). */
...
export async function fundCompute(broker: any, provider: string): Promise<void> {
  ...
  try { await broker.ledger.depositFund(1.0); } catch { /* ledger may already be funded */ }
  try { await broker.inference.acknowledgeProviderSigner(provider); } catch { /* idempotent */ }
  try { await broker.ledger.transferFund(provider, "inference", 1_000_000_000_000_000_000n); } catch { /* already funded */ }
}
 ...
 */
export async function generate(broker: any, svc: ImageService, baseBytes: Buffer, prompt: string): Promise<GenerationResult> {
  const attempt = async (): Promise<GenerationResult> => {
    ...
    form.append("image", new Blob([baseBytes as unknown as BlobPart], { type: "image/png" }), "image.png");
